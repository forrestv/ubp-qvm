...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { &g_suddenDeathTime, "g_suddenDeathTime", "15", CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, 0, qtrue },
  ...
  { &g_friendlyFireAttackerFrac, "g_friendlyFireAttackerFrac", "1", CVAR_ARCHIVE | CVAR_SERVERINFO, 0, qtrue  },
  { &g_friendlyFireVictimFrac, "g_friendlyFireVictimFrac", "0", CVAR_ARCHIVE | CVAR_SERVERINFO, 0, qtrue  },
  ...
  { &g_killerHP, "g_killerHP", "1", CVAR_ARCHIVE, 0, qfalse  },
...
void CheckTeamVote( int team )
{
  ...
  {
    if( level.teamVoteYes[ cs_offset ] > level.teamVoteNo[ cs_offset ] && level.teamVoteYes[ cs_offset ] >= 1 )
    {
      // execute the command, then remove the vote
      G_TeamCommand( team, va("print \"Team vote passed  (%d - %d)\n\"", level.teamVoteYes[ cs_offset ], level.teamVoteNo[ cs_offset ] ) );
      trap_SendConsoleCommand( EXEC_APPEND, va( "%s\n", level.teamVoteString[ cs_offset ] ) );
      pass = qtrue;
    }
    else
    {
      G_TeamCommand( team, va("print \"Team vote failed  (%d - %d)\n\"", level.teamVoteYes[ cs_offset ], level.teamVoteNo[ cs_offset ] ) );
